...
var wanted_brwstats_fields = []*mapping{
	{
...
func (l *Lustre2) GetLustreProcBrwStats(fileglob string, wantedFields []*mapping, acc telegraf.Accumulator) error {
	files, err := filepath.Glob(fileglob)
	if err != nil {
		return err
	}
...
func (l *Lustre2) Gather(acc telegraf.Accumulator) error {
	...
	for _, procfile := range l.Ost_procfiles {
		if strings.HasSuffix(procfile, "brw_stats") {
			err := l.GetLustreProcBrwStats(procfile, wanted_brwstats_fields, acc)
			if err != nil {
				return err
			}
		} else if strings.HasSuffix(procfile, "job_stats") {
			err := l.GetLustreProcStats(procfile, wanted_ost_jobstats_fields, acc)
			if err != nil {
				return err
			}
		} else {
	...
	for _, procfile := range l.Mds_procfiles {
		if strings.HasSuffix(procfile, "brw_stats") {
			...
			err := l.GetLustreProcStats(procfile, wanted_mdt_jobstats_fields, acc)
			if err != nil {
				return err
			}
		} else {
